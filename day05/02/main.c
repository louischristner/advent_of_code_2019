...
char *read_input(char *input)
{
    ...
    char *buf = malloc(sizeof(char) * 4086);

    if (fd == -1 || buf == NULL)
        return NULL;
    buf[read(fd, buf, 4085)] = '\0';
    close(fd);
    return buf;
}
...
int add(int **tab, int pos)
{
    ...
    printf("%d : %d, %d, %d, %d\n", pos, (*tab)[pos], (*tab)[pos + 1], (*tab)[pos + 2], (*tab)[pos + 3]);
...
int jmp_if_true(int **tab, int pos)
{
    int instr = (*tab)[pos];
    int first = ((instr / 100) % 10) ? (*tab)[pos + 1] : (*tab)[(*tab)[pos + 1]];
    int second = ((instr / 1000) % 10) ? (*tab)[pos + 2] : (*tab)[(*tab)[pos + 2]];

    printf("%d : %d, %d, %d\n", pos, (*tab)[pos], (*tab)[pos + 1], (*tab)[pos + 2]);
    if (first)
        return second - pos;
    return 3;
}

int jmp_if_false(int **tab, int pos)
{
    int instr = (*tab)[pos];
    int first = ((instr / 100) % 10) ? (*tab)[pos + 1] : (*tab)[(*tab)[pos + 1]];
    int second = ((instr / 1000) % 10) ? (*tab)[pos + 2] : (*tab)[(*tab)[pos + 2]];
...
int *compute(int *tab, int len)
{
    int act = 0;
    int opcode = 0;
    int select[] = {1, 2, 3, 4, 5, 6, 7, 8};
    int (*ptr[])(int **, int) = {add, mul, in, out, jmp_if_true, jmp_if_false, less_than, equal};
